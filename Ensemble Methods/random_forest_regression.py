...
def get_data():
  ...
  df = pd.read_csv('large_files/housing.data', header=None, delim_whitespace=True, engine='python')
  df.columns = [
    'crim', # numerical
    'zn', # numerical
    'nonretail', # numerical
...
if __name__ == '__main__':
  ...
  print("CV baseline - Linear Regression:", cross_val_score(baseline, Xtrain, Ytrain).mean())
  print("CV single tree:", cross_val_score(single_tree, Xtrain, Ytrain).mean())
  print("CV Random Forest:", cross_val_score(model, Xtrain, Ytrain).mean())

  # test score
  single_tree.fit(Xtrain, Ytrain)
  baseline.fit(Xtrain, Ytrain)
  print("Test score baseline - Linear Regression:", baseline.score(Xtest, Ytest))
  print("Test score single tree:", single_tree.score(Xtest, Ytest))
  print("Test score Random Forest:", model.score(Xtest, Ytest))
